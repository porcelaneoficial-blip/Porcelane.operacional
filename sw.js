const CACHE = 'porcelane-operacional-v5';
...
(() => {
  ...
  const renderBudgetActions = () => {
    ...
    tbody.querySelectorAll('[data-approve-budget]').forEach(button => button.addEventListener('click', async () => {
      button.disabled = true;
      try { await approveBudget(button.dataset.approveBudget); } finally { button.disabled = false; renderBudgetActions(); }
    }));
  };
  ...
  const renderRealPanels = async () => {
    try {
      ...
      if(estoqueBody){
        const {data,error}=await supabase.from('materiais').select('nome,categoria,unidade,estoque,estoque_minimo,preco').eq('ativo',true).order('nome');
        if(!error) estoqueBody.innerHTML=(data||[]).map(m=>`<tr><td>${clean(m.nome)}</td><td>${clean(m.categoria||'—')}</td><td>${Number(m.estoque||0)}</td><td>${clean(m.unidade||'—')}</td></tr>`).join('')||'<tr><td colspan="4" class="empty">Nenhum material cadastrado.</td></tr>';
      }
