...
const ASSUNTO_FILA = [
  ['orçamento','vendas'], ['orcamento','vendas'], ['preço','vendas'], ['valor','vendas'], ['pedido','vendas'],
  ['pagamento','financeiro'], ['parcela','financeiro'], ['pix','financeiro'], ['boleto','financeiro'], ['comprovante','financeiro'],
  ['medição','medicao'], ['medicao','medicao'], ['conferência','medicao'], ['conferencia','medicao'],
  ['produção','producao'], ['producao','producao'], ['corte','producao'],
  ['acabamento','acabamento'], ['polimento','acabamento'],
  ['instalação','instalacao'], ['instalacao','instalacao'], ['montagem','instalacao'], ['entrega','instalacao'],
  ['reclamação','pos_venda'], ['reclamacao','pos_venda'], ['assistência','pos_venda'], ['assistencia','pos_venda'], ['garantia','pos_venda']
];
...
async function load() {
  const f = await db.from('whatsapp_filas').select('id,codigo,nome,descricao,ativa').eq('ativa', true).order('nome');
  if (f.error) { console.warn('WhatsApp filas:', f.error.message); return; }
  filas = f.data || [];
  const c = await db.from('whatsapp_conversas').select('id,telefone,nome_cliente,cliente_id,pedido_id,fila_id,assunto,status,responsavel_id,ultima_mensagem_em,nao_lidas').order('ultima_mensagem_em',{ascending:false});
  if (c.error) { console.warn('WhatsApp conversas:', c.error.message); return; }
  conversas = c.data || [];
  render();
}
