...
(() => {
  ...
  const renderBudgetActions = () => {
    const tbody = document.getElementById('budgets-table'); if(!tbody || typeof budgets === 'undefined') return;
    ...
    tbody.innerHTML = budgets.map(b => { const approved=b.status==='aprovado', refused=b.status==='recusado'; const action=approved?'<span class="badge green">Pedido gerado</span>':refused?'<span class="badge">Recusado</span>':`<button class="primary" data-approve-budget="${clean(b.id)}">Aprovar e gerar pedido</button>`; return `<tr><td>${clean(b.id)}</td><td>${clean(b.client)}</td><td>${clean(b.job)}</td><td>${b.area?b.area.toLocaleString('pt-BR')+' m²':'—'}</td><td>${money(b.value)}</td><td><span class="badge ${approved?'green':'amber'}">${clean(b.status)}</span></td><td>${action}</td></tr>`; }).join('');
